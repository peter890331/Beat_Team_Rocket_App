...
3. After granting permissions, you are ready to start using the script.
  ...
  *Select the attributes of the Team Rocket grunts you want to challenge.*
   ...
   \- 啟動腳本
   確認完成後，直接點擊下方啟動！  
   *After confirming are complete, click below to start!*

  #### Floating Control Panel
  After clicking Start, the app will request Screen Capture permission (used for OpenCV image recognition), just allow it, then both GPS JoyStick and Pokémon GO will launch automatically, and a floating window will appear on your screen, the descriptions of floating panel as follows:

   - 主圖示 (Main Icon)：  
     長按可上下拖曳懸浮窗位置。  
     *Long press to drag the position up and down of the floating window.*

   - ▶ 啟動 (Play)：  
     確認遊戲載入完畢，並將視角拉至最小後，點擊此按鈕正式開始掛機。  
     *Make sure the game is fully loaded and the perspective is minimized, then click this button to officially start botting.*
   ...
   - 📸 截圖 (Screenshot)：  
     手動截取當前畫面並儲存。  
     *Manually capture the current screen and save it.*
